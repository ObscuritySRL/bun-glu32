...
import type { GLUquadric, GLUquadricErrorProc, GLUnurbs, GLUnurbsErrorProc, GLUtessCallbackProc, GLUtesselator, GLboolean, GLdouble, GLdouble_, GLenum, GLfloat, GLfloat_, GLint, GLint_, GLubyte_, GLvoid_, GLWChar_ } from '../types/GLU32';
...
class GLU32 {
  /**
   * Lazily binds a `glu32.dll` export, memoizing it on the class for future calls.
   *
   * Each method defined below uses `Load` on first invocation. When an export has
   * already been bound, the cached symbol is returned directly.
   *
   * @param method Exact name that exists in `Symbols`.
   * @returns The native function typed to the public wrapper.
   */
  private static Load<T extends keyof typeof GLU32.Symbols>(method: T): (typeof GLU32)[T] {
    const skip = Object.getOwnPropertyDescriptor(GLU32, method)?.configurable === false;

    if (skip) {
      return GLU32[method];
    }

    const library = dlopen('glu32.dll', { [method]: GLU32.Symbols[method] });
    const propertyDescriptor = { configurable: false, value: library.symbols[method] };

    Object.defineProperty(GLU32, method, propertyDescriptor);

    return GLU32[method];
  }

  /**
   * Binds a subset (or all) of the `glu32.dll` exports ahead of time.
   *
   * Already-bound symbols are skipped; pass a list to warm only what you need
   * for hot paths. Omitting the argument binds every entry in `Symbols`.
   *
   * @param methods Optional slice of method names to bind.
   */
  public static Preload(methods?: (keyof typeof GLU32.Symbols)[]): void {
    methods ??= Object.keys(GLU32.Symbols) as (keyof typeof GLU32.Symbols)[];

    const symbols = Object.fromEntries(methods.filter((method) => Object.getOwnPropertyDescriptor(GLU32, method)?.configurable !== false).map((method) => [method, GLU32.Symbols[method]]));

    const library = dlopen('glu32.dll', symbols);

    const propertyDescriptorMap = Object.fromEntries(Object.entries(library.symbols).map(([key, value]) => [key, { configurable: false, value }]));

    Object.defineProperties(GLU32, propertyDescriptorMap);

    return;
  }

  /**
   * Raw symbol map for `dlopen('glu32.dll', Symbols)`.
   *
  ...
  public static gluTessBeginPolygon(tess: GLUtesselator, polygon_data: GLvoid_): void {
